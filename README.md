# Lee Seou

Industrial Engineering · Sungkyunkwan University (SKKU)
sho515@g.skku.edu · [github.com/Seou-Web](https://github.com/Seou-Web)

---

## Skills

- **Tools:** Python, MATLAB
- **Numerical Methods:** LU factorization, Gauss-Seidel, Runge-Kutta, FDM, FFT, interpolation
- **Data Analysis:** ANOVA, statistical analysis

---

## Capstone Design Project

### Visualization Simulator for Cosmetic Manufacturing Vessels Using Manufacturing Data
*Smart Factory Capstone Design 1 · Team 12*

Developed a Population Balance Model (PBM)-based simulator with multiple quality indicators to predict and judge scale-up emulsification quality in cosmetic emulsion manufacturing vessels (CEVSim).
- Responsible for results analysis and project management
- Analyzed droplet size distribution data to evaluate emulsion quality and predict manufacturing outcomes
- Project outcomes resulted in a pending patent application and a registered software work (see *Research Outcomes* below)

---

## Research Outcomes

### Patent Application (Under Examination)
**Method and System for Predicting and Determining Scale-up Emulsification Quality of Cosmetic Emulsion Manufacturing Vessels Using a Population Balance Model and Multiple Quality Indicators**
- Applicant: Sungkyunkwan University Industry-Academic Cooperation Foundation
- Inventors: Jeong Jongpil, Kang Mingyu, **Lee Seou**, Lee Hakseung
- Reference No.: R-2026-0578-KR-1 · Status: Under Examination

### Software Registration (Registered)
**CEVSim (Cosmetic Emulsification Vessel Scale-Up Simulator)**
- Type: Computer Program Work — Application Software, Industrial S/W
- Authors: Jeong Jongpil, Yoo Jaekyung, Kang Mingyu, **Lee Seou**, Lee Hakseung
- Registration No.: C-2026-030863 · Accession No.: 2026-032760
- Status: Registered June 23, 2026 (created May 29, 2026)

---

## Research Papers

### Team Capstone Manuscript (Submitted, double-blind review)
**Scale-Up Simulation of Cosmetic Emulsion Manufacturing Vessels (100/300/500 L) Using a Population Balance Model: Integrated Multi-Metric Analysis of Droplet Size Distribution, Uniformity, Viscosity, and Hardness**
Jeong Jongpil, Yoo Jaekyung, Kang Mingyu, **Lee Seou**, Lee Hakseung — manuscript prepared from the capstone project results (2026).

### Individual Target Paper Review (Mar. 2026)
**CFD simulation of a Rushton turbine stirred-tank using open-source software with critical evaluation of MRF-based rotation modeling**
Reid, A., Rossi, R., Cottini, C., Benassi, A. — *Meccanica* (Springer), 2025, pp. 1613–1637.

### Individual Open-Source Analysis (May 2026)
**BioSTEAM: A Fast and Flexible Platform for the Design, Simulation, and Techno-Economic Analysis of Biorefineries under Uncertainty**
Cortés-Peña, Y. et al. — *ACS Sustainable Chemistry & Engineering*, 2020, 8(8), 3302–3310.

### Team Open-Source Paper Analysis (May 2026 · Team 12)
**A population balance model for cosmetic emulsion design: A multiscale approach**
Calvo, F., Gómez, J. M., Ricardez-Sandoval, L., Álvarez, O. — *Chemical Engineering Science*, 2024, 287: 119737.

---

## Education

**B.S. in Industrial Engineering** — Sungkyunkwan University
2025.03 ~ Present
